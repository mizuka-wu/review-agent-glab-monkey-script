...
const TOOL_SYSTEM_PROMPT_BASE = `你是 GitLab 代码评审助手。你可以调用工具来获取仓库中更多上下文信息，以做出更准确的评审。
...
function buildToolSystemPrompt(tools: ToolDefinition[]): string {
  ...
  let prompt = TOOL_SYSTEM_PROMPT_BASE;
  ...
  return prompt + TOOL_SYSTEM_PROMPT_SUFFIX;
}
...
export async function runAgentLoop(
  ...
  while (iterations < maxIterations) {
    ...
    try {
      response = await runtime.callWithTools(messages, tools, buildToolSystemPrompt(tools), { signal: options.signal });
    } catch (error) {
