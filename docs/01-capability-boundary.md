...
本文回答一个核心问题：哪些功能油猴脚本可以直接做，哪些必须依赖 GitLab API，哪些需要本地服务。

> **当前范围**：纯浏览器端实现，无本地 Gateway / CI Bot。需要服务端或本地进程的能力已明确排除。

## 1. 总体结论

| 能力 | 油猴脚本 | GitLab API / 页面 | 结论 |
| --- | --- | --- | --- |
| 页面注入、划词、悬浮工具栏 | 直接实现 | 不需要 | 纯浏览器可做 |
| 读取选中文本、文件名、当前 URL | 直接实现 | DOM 辅助 | 纯浏览器可做 |
| Chatbox、会话 UI、Markdown 渲染 | 直接实现 | 不需要 | 纯浏览器可做 |
| 读取 MR 标题、描述、变更文件、Diff | 可发起请求 | 需要 REST | 浏览器 + GitLab API |
| 读取完整文件、历史版本 | 可按需请求 | 需要 API | 浏览器可做 |
| Agent 工具循环（file_read/search/git_log） | 可发起请求 | 需要 API | 浏览器可做 |
| 调用 OpenAI/Anthropic/Gemini 模型 | 直连 | 不相关 | 浏览器可做 |
| 企业网关认证（自定义 Header/Query） | 直接实现 | 不相关 | 浏览器可做 |
| MCP 扩展工具（Streamable HTTP） | GM.xmlHttpRequest | 不相关 | 浏览器可做 |
| 仓库级符号搜索、调用链 | 不可靠 | API 能力有限 | 需要本地进程（排除） |
| 运行测试、lint、构建 | 不可做 | 不可做 | 需要本地进程（排除） |
| 精确行评论发布 | 可调用 | 需要 Discussions API | 浏览器可做，必须确认 |
| 评论去重、幂等更新 | 可实现 | 需要读取 Discussions | 浏览器可做 |
| 团队规则、规则包管理 | 可存本地配置 | — | 浏览器可做 |
| 密钥安全保存 | 混淆存储 | — | 浏览器可做 |
